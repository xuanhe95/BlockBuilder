...
public partial class Group<P, T>
{
    ...
    // private Dictionary<T, Type<T>> ModMap;
    ...
    // {
    //     foreach(T go in ModMap.Keys){
    //         RuleGenerator rg = (RuleGenerator) Go.GetComponent<RuleCreator>();
    //         if(rg == null) continue;
    //         foreach(T relative in rg.H){
    //             int direction = CheckBackToBack(ModMap[go].GetName());
    //             switch (direction)
    //             {
    //                 case Direction.Left:
